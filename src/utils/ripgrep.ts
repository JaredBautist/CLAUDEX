...
const getRipgrepConfig = memoize((): RipgrepConfig => {
  ...
  const vendorCmd =
    process.platform === 'win32'
      ? path.resolve(rgRoot, platformDir, 'rg.cmd')
      : ''

  const stubPath = path.resolve(rgRoot, 'ripgrep-stub.js')

  const command: string = (() => {
    if (existsSync(vendorExe)) return vendorExe
    if (process.platform === 'win32' && existsSync(vendorCmd)) return vendorCmd
    if (existsSync(stubPath)) return process.execPath
    return vendorExe
  })()
  const args: string[] =
    !existsSync(vendorExe) &&
    !(process.platform === 'win32' && existsSync(vendorCmd)) &&
    existsSync(stubPath)
      ? [stubPath]
      : []
...
function ripGrepRaw(
  ...
  // In this sandboxed Windows env, spawning binaries can be blocked (EPERM).
  // Short-circuit to a no-op child to let the CLI continue without ripgrep.
  if (process.env.CLAUDE_CODE_DISABLE_RIPGREP === '1' || getPlatform() === 'windows') {
...
export async function ripGrep(
  ...
  if (process.env.CLAUDE_CODE_DISABLE_RIPGREP === '1' || getPlatform() === 'windows') {
    logForDebugging('[ripgrep] disabled, returning empty results')
    return []
  }
  if (process.env.CLAUDE_CODE_SIMPLE === '1') {
    logForDebugging('[ripgrep] skipping ripGrep because CLAUDE_CODE_SIMPLE=1');
    return [];
  }
  await codesignRipgrepIfNecessary()
...
export const countFilesRoundedRg = memoize(
  async (
    ...
    if (process.env.CLAUDE_CODE_SIMPLE === '1') {
      logForDebugging(
        '[ripgrep] skipping countFilesRoundedRg because CLAUDE_CODE_SIMPLE=1',
      )
      return 0
    }
